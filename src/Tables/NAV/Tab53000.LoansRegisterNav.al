...
Table 53000 "Loans Register Nav"
{
    // DrillDownPageID = 51501;
    // LookupPageID = UnknownPage51516501;
    ...
    fields
    {
        ...
        field(3; "Loan Product Type"; Code[250])
        {
            ...
            begin
                ...
                if Cust.FindFirst then begin
                    ...
                    //CHECK ON THIS LATER
                    // if (Cust."Shares Retained" * -1 < GenSetUp."Retained Shares") and ("Loan Product Type" <> 'QUICK') then
            ...
            begin
                ...
                if LoanType.Get("Loan Product Type") then begin
                    if LoanType."Maximum No. Of Runing Loans" > 1 then begin
                        ...
                        //      LoanBal:=SURESTEPFactory.KnGetLoanBalanceOneLoan(LoansRec."Loan  No.");
        ...
        {
            CalcFormula = sum("Member Ledger Entry".Amount where("Customer No." = field("Client Code"),
                                                                  "Transaction Type" = filter("Unallocated Funds" | "Loan"),
                                                                  "Loan No" = field("Loan  No."),
                                                                  "Posting Date" = field("Date filter")));
            Editable = false;
            FieldClass = FlowField;
        ...
        {
            TableRelation = "Loans Register Nav"."Loan  No.";
        }
        ...
        {
            CalcFormula = sum("Loans Register Nav"."Loan Principle Repayment" where("Client Code" = field("Client Code"),
                                                                                 "Outstanding Balance" = filter(> 1)));
            FieldClass = FlowField;
        }
        field(69034; "Total Loan Interest"; Decimal)
        {
            CalcFormula = sum("Loans Register Nav"."Loan Interest Repayment" where("Client Code" = field("Client Code"),
                                                                                "Outstanding Balance" = filter(> 1)));
            FieldClass = FlowField;
        }
        ...
        {
            CalcFormula = sum("Member Ledger Entry".Amount where("Customer No." = field("Client Code"),
                                                                  "Transaction Type" = filter("Loan" | "Loan"),
                                                                  "Loan No" = field("Loan  No.")));
            FieldClass = FlowField;
        }
        ...
        {
            Enabled = false;
            TableRelation = "Loans Register Nav"."Loan  No." where("Client Code" = field("Client Code"));
        ...
        {
            TableRelation = "Loans Register Nav"."Loan  No." where("BOSA No" = field("BOSA No"),
                                                                Posted = filter(true));
        ...
        {
            TableRelation = "Loans Register Nav"."Loan  No." where("Approval Status" = filter(Approved),
                                                                "Outstanding Balance" = filter(0),
        ...
        {
            DataClassification = ToBeClassified;
            TableRelation = "Loans Register Nav"."Loan  No." where("Loan Status" = const(Approved),
                                                                Posted = const(false),
                                                                "Client Code" = field("Client Code"));
        }
    ...
    begin
        ...
        if Users.Find('-') then begin
            "Cashier Branch" := 'Nairobi';// Users."Branch Code";
            //MODIFY;
        end;
    ...
    var
        ...
        ProdCycles: Record 51384;
        LoanApp: Record "Loans Register Nav";
        MemberCycle: Integer;
        ...
        LBalance: Decimal;
        UsersID: Record User;
        LoansBatches: Record 51377;
        Employer: Record 51401;
        GenSetUp: Record 51398;
        Batches: Record 51377;
        MovementTracker: Record 51394;
        SpecialComm: Decimal;
        CustR: Record 51364;
        RAllocation: Record 51387;
        "Standing Orders": Record 51449;
        StatusPermissions: Record 51452;
        CustLedg: Record 51365;
        LoansClearedSpecial: Record 51379;
        BridgedLoans: Record 51379;
        Loan: Record "Loans Register Nav";
        ...
        Loans: Record "Loans Register Nav";
        Chargeable: Decimal;
        Saccodeduct: Decimal;
        SaccoDedInt: Decimal;
        LoanAppeal: Record "Loans Register Nav";
        HREmp: Record 51364;
        LoansRec: Record "Loans Register Nav";
        TotalLoanOutstanding: Decimal;
        LineNoG: Integer;
        LoansG: Record 51372;
        ObjProductDepositLoan: Record 51709;
        ObjLoansRec: Record "Loans Register Nav";
        ObjCellGroup: Record 51915;
        ObjGuarantors: Record 51372;
        ObjCust: Record 51364;
        ObjProductCharge: Record 51383;
        LInsurance: Decimal;
        ObjDepositHistory: Record 51916;
        ObjAccountLedger: Record 51365;
        ObjStatementB: Record 51916;
        ...
        ObjLoansRec2: Record "Loans Register Nav";
        ObjLoanOffsets: Record 51376;
        VarTotalMonthlyRepayments: Decimal;
        ObjUser: Record User;
        VarTranchNo: Integer;
        ObjGradInt: Record 51404;
        ...
        totalLn: Decimal;
        LoanCount: Integer;
        LoanApps: Record "Loans Register Nav";
        DisbAmt: Decimal;
        EFTFee: Decimal;
