...
codeunit 51023 "Paybill Processor"
{
    ...
    begin
        ...
        paybillTransTable.SetAscending(TransDate, false);
        ...
        if paybillTransTable.FindSet() then begin
            ...
            if CheckGeneralLinesPosted(thisDocNum) then begin
                ...
                Result := 'Transaction already posted on GL Table';
                exit;
            end;
                ...
                CASE UPPERCASE(paybillTransTable."Key Word") OF
                    ...
                    ELSE
                        ...
                        if memberNumber <> '' then begin
                            ...
                            IF STRLEN(thisAccNum) <= 8 THEN BEGIN
                                vendorTable.RESET;
    ...
    local procedure fnMemberPostings(memberNumber: Code[120]; thisAmount: Decimal; thisDocNum: Code[30]) Result: Text;
    var
        ...
        postingAmount: Decimal;
        documentno: Code[30];
        insuranceContributions: Decimal;
    ...
    begin
        ...
        insuranceContributions := fnGetInsuranceDeductions(memberNumber);
        if postingAmount < insuranceContributions then insuranceContributions := postingAmount;

        LineNo := LineNo + 10000;
        CreateGenJournalLine(jnlBatchTemplate, jnlBatchName, LineNo,
        documentno, "Gen. Journal Account Type"::Customer,
        memberNumber, memberNumber, 'Paybill deposit to insurance Contributions ' + memberNumber,
        -1 * insuranceContributions, Today, TransactionTypesEnum::"Insurance Contribution");

        postingAmount := postingAmount - insuranceContributions;
    ...
    procedure fnGetMemberNumber(parameter: Code[130]) memberNumber: Code[30]
    begin
        memberNumber := '';
    ...
    local procedure FnPostLoansBal(memberNumber: Code[120]; parsedAmount: Decimal; thisDocNum: Code[30]) remainingAmount: Decimal
    var
        loanNumber: Code[50];
    begin
        ASAT := Today;
    ...
    local procedure ApplyLoanCode(var parsedAmount: Decimal; memberNumber: Code[120]; loanCode: Code[10]; thisDocNum: Code[30])
    var
        loanBal: Decimal;
    ...
    begin
        ...
        if membersTable.FindFirst() then begin
            ...
            if membersTable."Shares Retained" >= 15000 then
                ...
                if membersTable."Shares Retained" >= 5000 then
                    ShareCapital := 417
                else
                    ShareCapital := 1000;
        end;
    ...
    procedure fnGetInsuranceDeductions(memberNumber: Code[30]) insuranceDeduc: Decimal
    var
    ...
    begin
        ...
        customerLedEntries.SetRange("Customer No.", memberNumber);
        customerLedEntries.SetRange(
            "Transaction Type",
            customerLedEntries."Transaction Type"::"Insurance Contribution");

        // Current month only
        customerLedEntries.SetRange("Posting Date", startDate, endDate);

        // Exclude reversed entries
        customerLedEntries.SetRange(Reversed, false);

        // Sum posted amounts
        customerLedEntries.CalcSums("Amount Posted");
        insurancePaid := ABS(customerLedEntries."Amount Posted");

        // Remaining balance to deduct
        if insurancePaid >= insuranceDeduc then
            insuranceDeduc := 0
        else
            insuranceDeduc := insuranceDeduc - insurancePaid;
