...
codeunit 56701 LoanAgingProcessor
{
    ...
    procedure fnAgingStatement(ASAT: Date) exitString: Text
    var
        Filename: Text[100];
    ...
    begin
        LoansRegister.Reset();
        LoansRegister.SetFilter("Date filter", '..' + Format(Today));
        if LoansRegister.Find('-') then begin
            RecRef.GetTable(LoansRegister);
            Clear(TempBlob);
            TempBlob.CreateOutStream(Outstr);
            TempBlob.CreateInStream(Instr);
            if Report.SaveAs(Report::"loan aging new 2026", '', ReportFormat::Pdf, Outstr, RecRef) then begin
    ...
    ////===================
    procedure GenerateLoanAging(AsAt: Date): Text
    var
        TempBlob: Codeunit "Temp Blob";
        Base64Convert: Codeunit "Base64 Convert";
        OutStr: OutStream;
    ...
    begin
        ...
        // Pass request page parameter
        ReportParameters :=
        '<?xml version="1.0" standalone="yes"?>' +
        '<ReportParameters name="loan aging new 2026">' +
            '<DataItems>' +
            '</DataItems>' +
            '<Options>' +
                '<Field name="AsAt">' + Format(AsAt, 0, 9) + '</Field>' +
            '</Options>' +
        '</ReportParameters>';

        Report.SaveAs(
            Report::"loan aging new 2026",
            ReportParameters,
            ReportFormat::Excel,
            OutStr
        );
