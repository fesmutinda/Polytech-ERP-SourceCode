...
Report 56372 "Loans Register  CIC"
{
    DefaultLayout = RDLC;
    RDLCLayout = './Layouts/Loans Register  CIC.rdlc';

    dataset
    {
        dataitem("Loans Register"; "Loans Register")
        {
            DataItemTableView = sorting("Loan  No.") order(ascending) where(Posted = filter(true), "Loan GL Insurance" = filter(false));
            PrintOnlyIfDetail = false;
            RequestFilterFields = "Issued Date", "Loan  No.", "Loan Product Type";
            ...
            column(CompanyLetter_Head; Company."Address 2")
            {
            }
    ...
    begin
        if "COMPY INFOR".Get then begin
            "COMPY INFOR".CalcFields("COMPY INFOR".Picture);///, "COMPY INFOR".Letter_Head);
