...
Report 50257 "Loan Guard Report"
{
    ...
    dataset
    {
        ...
        {
            DataItemTableView = sorting("Staff No") order(ascending) where(Posted = const(true), "Outstanding Balance" = filter('>0'), "Loan Product Type" = filter('<>24')); //, "Loan Product Type" = filter('<>15&<>16&<>17&<>24'));          // RequestFilterFields = "Date filter", "Issued Date";
