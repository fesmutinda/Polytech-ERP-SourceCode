    ...
    dataset
    {
        dataitem("Loans Register"; "Loans Register")
        {
            ...
            dataitem("Default Notices Register"; "Default Notices Register")
            {
                ...
                column(Outstanding_Interest_DefaultNoticesRegister; IntBal)
                {
                }
            ...
            begin
                ...
                // Work directly with current record (no need to refetch Loans Register)
                RemainingPeriod := "Installments" - ObjSwizzFactory.KnGetCurrentPeriodForLoan("Loan  No.");

                // Get last loan repayment or interest payment date
                MemberLedgerEntry.RESET();
                MemberLedgerEntry.SETRANGE("Loan No", "Loan  No.");
                IF MemberLedgerEntry.FINDLAST() THEN BEGIN
                    IF (MemberLedgerEntry."Transaction Type" IN
                        [MemberLedgerEntry."Transaction Type"::"Loan Repayment", 
                        MemberLedgerEntry."Transaction Type"::"Interest Paid"]) THEN
                        LastPayDate := MemberLedgerEntry."Posting Date";
                END;

                IF LastPayDate = 0D THEN
                    LastPayDate := "Issued Date";

                // Ensure FlowFields are calculated
                CALCFIELDS("Outstanding Balance", "Oustanding Interest");

                // Calculate balances
                LBalance := "Outstanding Balance" + "Oustanding Interest";
                LBalance1 := "Outstanding Balance";
                Balance -= LBalance;
                SharesB -= LBalance;

                // Fetch repayment schedule balance up to today
                LoanRepaymentSchedule.RESET();
                LoanRepaymentSchedule.SETCURRENTKEY("Loan No.", "Repayment Date");
                ...
                VarArrearsAmount := PrincipleInArrears + "Oustanding Interest";
                ...
                // Final loan balance and commission calculations
                lbal := ROUND("Outstanding Balance", 0.5, '=');
                IntBal := "Oustanding Interest";
                COMM := IntBal * 0.5;
            end;
